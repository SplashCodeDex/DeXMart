...
const cleanLog = logContent.replace(/[\u001b\u009b][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><]/g, '');
...
const categories = {
  A: {
    ...
    patterns: ['src/agents/', 'src/cron/', 'src/auto-reply/', 'src/browser/', 'src/infra/', 'src/commands/', 'src/config/', 'src/gateway/'],
    files: [],
    rootCause: 'These are OpenClaw upstream tests that depend on internal mocking infrastructure and test utilities that weren\'t fully ported during the Phase 1 repository restructure.',
  ...
  B: {
    ...
    files: [],
    rootCause: 'Mixed — webhook tests time out at 5000ms, createTelegramBot mock returns incompatible shape, resolveTelegramFetch tests expect specific undici dispatcher behavior.',
  ...
  C: {
    ...
    files: [],
    rootCause: 'getLastSocket() throws Invalid Baileys socket getter because session.ts changed socket creation flow in Phase 5.',
    remediation: 'Update test-helpers.ts getLastSocket() to handle the new SaaS-mode socket creation path.'
  },
  D: {
    name: 'Stale DeXMart Service Tests',
    patterns: ['src/services/IngressService', 'authMiddleware', 'jobs/index', 'flowEngine.skill', 'src/controllers/', 'src/routes/channelLifecycle'],
    files: [],
    rootCause: 'Tests assert pre-Phase 4/5 behavior (e.g., IngressService expects unifiedAI.processMessage()).',
  ...
  E: {
    ...
    files: [],
    rootCause: 'DM policy tests and security audit tests expect configurations/interfaces that changed in OpenClaw upstream.',
    remediation: 'Verify current DM policy behavior → update test expectations.'
  },
  F: {
    name: 'Miscellaneous Channel Tests',
    patterns: ['src/signal/', 'src/slack/', 'src/line/', 'src/discord/', 'src/facebook/', 'src/wizard/', 'src/tools/cmd.test.ts', 'src/analytics/', 'src/hooks/'],
    files: [],
...
for (const file of failingFiles) {
  ...
  if (!categorized && file.includes('test-helpers.ts')) { categories.C.files.push(file); categorized = true; }
  if (!categorized && ['session.test.ts', 'session'].some(p => file.includes(p) && file.includes('src/web/'))) { categories.C.files.push(file); categorized = true; }
