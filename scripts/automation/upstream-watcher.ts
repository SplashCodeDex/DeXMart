...
const INJECTION_POINTS = [
  ...
  {
    file: "src/ingress/ingress-service.ts",
    keywords: ["ingress-service", "IngressService", "runEmbeddedPiAgent", "processMessage"],
  },
  ...
  // Phase 5 engine injection points
  {
    ...
    keywords: ["PluginRegistry", "PluginRuntime", "registerPlugin"],
  },
  {
    file: "gateway/server-channels.ts",
    keywords: ["server-channels", "createChannelManager", "channelManager"],
  },
 ...
 */
async function fetchAllReleasesInRange(
  from: ParsedVersion,
  to: ParsedVersion,
...
function parseChangelog(body: string): ParsedChangelog {
  ...
  if (result.breaking.length === 0) {
    const breakingInline = [...result.changes].filter(
      (c) => c.toLowerCase().includes("**breaking**") || c.toLowerCase().includes("breaking:"),
    );
...
function generateGapAnalysisReport(reports: VersionReport[], from: string, to: string): string {
  ...
  for (const r of stableVersions) {
    const relatedBetas = betaVersions.filter(
      (b) =>
  ...
  for (const r of reports) {
    ...
    if (
      r.changelog.changes.length === 0 &&
      r.changelog.fixes.length === 0 &&
      r.changelog.breaking.length === 0
    ) {
...
async function runGapAnalysis(fromTag: string, toTag: string, noBetas: boolean, dryRun: boolean) {
  ...
  for (const rel of releases) {
    ...
    const alertIcon =
      injectionPointHits.length > 0 ? "🎯" : changelog.breaking.length > 0 ? "⚠️" : "✅";
    console.log(
      `   ${alertIcon} ${rel.tag_name}: ${changelog.changes.length}C / ${changelog.breaking.length}B / ${changelog.fixes.length}F`,
    );
  }
  ...
  if (injectionConflicts > 0) {
    console.log(
      `   🎯 ${injectionConflicts} versions touch DeXMart injection points — manual conflict resolution required.`,
    );
  }
}
