    ...
    return (
      <div className="space-y-4">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="h-24 w-full bg-card border border-border rounded-2xl animate-pulse"
          />
        ))}
      </div>
    );
      ...
      <div className="relative pl-8 space-y-8 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-border/50">
        {checkpoints.map((cp, index) => (
          <div key={cp.id} className="relative">
            <div
              className={`absolute -left-[30px] top-1 w-5 h-5 rounded-full border-4 border-background z-10 ${index === 0 ? "bg-primary" : "bg-muted-foreground/30"}`}
            />

            <Card
              className={`overflow-hidden border-border/50 hover:border-primary/30 transition-colors group ${selectedCheckpoint === cp.id ? "ring-2 ring-primary/20 border-primary/30" : ""}`}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-black uppercase tracking-widest">
                      {cp.label || `Checkpoint ${cp.id.slice(0, 6)}`}
                    </span>
                    <Badge
                      variant="outline"
                      className="text-[8px] font-black uppercase tracking-tighter px-1 py-0"
                    >
                      {cp.reason}
                    </Badge>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <div className="text-sm font-bold">
                      {cp.tokenCount?.toLocaleString() || "0"}
                    </div>
                  </div>
                  <div className="space-y-1">
                    <div className="text-[9px] font-black uppercase tracking-tighter text-muted-foreground">
                    ...
                    <div className="text-sm font-bold">
                      {cp.messageCount?.toLocaleString() || "0"}
                    </div>
                  </div>
                </div>
      ...
      <Dialog open={isRestoreDialogOpen} onOpenChange={setIsRestoreDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will overwrite the current session state with the selected checkpoint. Active
              runs will be interrupted. This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setIsRestoreDialogOpen(false)}>
              Cancel
            </Button>
      ...
      <Dialog open={isBranchDialogOpen} onOpenChange={setIsBranchDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new session starting from this checkpoint. The current session will remain
              unchanged.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setIsBranchDialogOpen(false)}>
              Cancel
            </Button>
