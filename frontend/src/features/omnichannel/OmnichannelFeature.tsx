    ...
    >
      <div
        className={cn(
          ...
          platform.bgColor + "/10",
          isGrid
            ? "rounded-full p-4 mb-4 group-hover:scale-110 transition-transform relative z-10"
            : "rounded-lg p-2 relative z-10",
        )}
      >
        <platform.Icon className={cn(platform.textColor, isGrid ? "h-8 w-8" : "h-4 w-4")} />
      </div>
      <span className={cn("font-medium relative z-10", isGrid ? "text-foreground" : "text-sm")}>
        {platform.label}
      </span>
...
function ChannelCard({ channel }: { channel: Channel }): React.JSX.Element {
  ...
  const agentName =
    agent?.name ??
    (effectiveAgentId === "system_default" ? "System Default Agent" : "Unknown Agent");
    ...
    <>
      <Card className="overflow-hidden border-border/50 bg-card/40 backdrop-blur-md transition-all hover:shadow-md h-full flex flex-col">
        ...
        <CardFooter className="bg-muted/30 border-t border-border/50 py-2 mt-auto flex gap-2">
          ...
          {(channel.status === "connected" || isConnecting) && !isTerminal ? (
            <Button
              variant="ghost"
              size="icon"
              className={cn(
...
export function OmnichannelFeature(): React.JSX.Element {
  ...
  const { channelAccounts, refresh: refreshRpc } = useChannelStatus({ interval: 10000 });
  ...
  // Merge RPC status into store channels
  const channels = storeChannels.map((channel) => {
    const accounts = channelAccounts[channel.type] || [];
    const rpcAccount = accounts.find(
      (a) => a.accountId === channel.account || a.name === channel.name,
    );

    if (rpcAccount) {
      return {
        ...channel,
        status: (rpcAccount.connected
          ? "connected"
          : rpcAccount.lastError
            ? "error"
            : channel.status) as Channel["status"],
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          ...
          <Button
            variant="secondary"
            asChild
            className="hidden sm:flex shadow-sm gap-2 border border-border/50 bg-card/60 hover:bg-muted font-medium text-foreground"
          >
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
            <DialogContent className="sm:max-w-[425px] p-0 border-none bg-transparent">
              <VisuallyHidden>
                ...
                <DialogDescription>Connect a new messaging platform to your bot.</DialogDescription>
              </VisuallyHidden>
      ...
      {isLoading && channels.length === 0 ? (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 w-full max-w-6xl mx-auto">
          {Array.from({ length: 8 }).map((_, i) => (
            <div
              key={i}
              className="flex flex-col items-center justify-center rounded-xl border border-border bg-card/30 p-6 min-h-[160px] animate-pulse"
            >
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div
            className={cn(
              ...
              channels.length >= 2 ? "lg:w-72" : "lg:w-[500px]",
            )}
          >
            ...
            <motion.div
              ...
              className={cn(
                "grid gap-3",
                channels.length >= 2 ? "grid-cols-2 lg:grid-cols-1" : "grid-cols-2",
              )}
            >
