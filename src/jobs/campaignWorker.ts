...
class CampaignWorker {
  ...
  public async processCampaign(job: Job<CampaignJobData>): Promise<void> {
    ...
    const currentCampaign = await firebaseService.getDoc<"users/{userId}/campaigns">(
      "campaigns",
      id,
      tenantId,
    );
    ...
    const templateResult = await TemplateService.getInstance().getTemplate(
      tenantId,
    ...
    if (!currentCampaign.stats.total) {
      await this.updateCampaignStats(tenantId, id, {
    ...
    const segmentSize =
      currentCampaign.antiBan.batchSize > 0 ? currentCampaign.antiBan.batchSize : 50;
    ...
    for (let i = sent + failed; i < endIndex; i++) {
      ...
      if (i % 5 === 0) {
        const live = await firebaseService.getDoc<"users/{userId}/campaigns">(
          "campaigns",
          id,
          tenantId,
        );
      ...
      try {
        const content = await this.prepareMessage(
          tenantId,
          template,
          contact,
          currentCampaign.antiBan.aiSpinning,
        );
        ...
        if (i < endIndex - 1) {
          const delay = Math.floor(
            gaussianRandom(currentCampaign.antiBan.minDelay, currentCampaign.antiBan.maxDelay) *
              1000,
          );
    ...
    if (sent + failed < total) {
      const delayTime =
        currentCampaign.antiBan.batchSize > 0
          ? Math.floor(
              gaussianRandom(
                currentCampaign.antiBan.batchPauseMin,
                currentCampaign.antiBan.batchPauseMax,
              ) *
                60 *
                1000,
            )
      ...
      await this.worker.queue.add(
        "process-campaign",
        { tenantId, campaign: currentCampaign },
        { delay: delayTime },
      );
    } else {
      await this.finalizeCampaign(tenantId, id, sent, failed, total);
  ...
  private async loadTargets(tenantId: string, audience: Campaign["audience"]): Promise<Contact[]> {
    if (audience.type === "audience") {
      const aud = await firebaseService.getDoc<"users/{userId}/audiences">(
        "audiences",
        audience.targetId,
        tenantId,
      );
      if (!aud) return [];
      const allContacts = await firebaseService.getCollection<"users/{userId}/contacts">(
        "contacts",
        tenantId,
      );
    ...
    if (audience.type === "groups") {
      ...
      if (audience.targetId === "all") {
        let groups = await firebaseService.getCollection<"users/{userId}/groups">(
          "groups",
          tenantId,
        );
        ...
        if (groups.length === 0 && activeChannelIds.length > 0) {
          try {
            ...
            if (socket) {
              ...
              groups = await firebaseService.getCollection<"users/{userId}/groups">(
                "groups",
                tenantId,
              );
            }
          } catch {
            /* runtime not yet initialised */
          }
        }
        ...
        let group = await firebaseService.getDoc<"users/{userId}/groups">(
          "groups",
          audience.targetId,
          tenantId,
        );
        ...
        if (!group && activeChannelIds.length > 0) {
          try {
            ...
            if (socket) {
              ...
              group = await firebaseService.getDoc<"users/{userId}/groups">(
                "groups",
                audience.targetId,
                tenantId,
              );
  ...
  private getAvailableChannelIds(
    tenantId: string,
  ...
  private async updateCampaignStats(
    ...
    await firebaseService.setDoc<"users/{userId}/campaigns">(
      "campaigns",
      campaignId,
      { stats: stats as any, updatedAt: new Date() },
      tenantId,
      true,
    );
  }
