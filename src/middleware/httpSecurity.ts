...
export const csrfProtection = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (origin) {
    ...
    if (origin !== allowedOrigin && !isLocalhost) {
      logger.warn(
        `Potential CSRF attack blocked: Origin mismatch. Origin: ${origin}, Expected: ${allowedOrigin}`,
      );
  ...
  if (!origin && referer) {
    try {
      ...
      const isLocalhost =
        refererUrl.origin.includes("localhost") || refererUrl.origin.includes("127.0.0.1");
      if (refererUrl.origin !== allowedOrigin && !isLocalhost) {
        logger.warn(
          `Potential CSRF attack blocked: Referer mismatch. Referer: ${referer}, Expected: ${allowedOrigin}`,
        );
        return res.status(403).json({ success: false, error: "Forbidden: CSRF protection" });
      }
    } catch (e) {
      ...
      return res
        .status(403)
        .json({ success: false, error: "Forbidden: CSRF protection (Invalid Referer)" });
    }
  }
