...
async function processExpiredCooldowns() {
  ...
  for (const metadata of expired) {
    try {
      ...
      if (autoResume) {
        logger.info(
          `[AntiBanResumeWorker] Auto-resuming campaign ${campaignId} for tenant ${tenantId}`,
        );
        ...
        if (campaign) {
          ...
          await (campaignWorker as any).worker.queue.add("process-campaign", {
            tenantId,
            campaign,
          });
        }
        ...
        logger.debug(
          `[AntiBanResumeWorker] Cooldown expired for ${campaignId}, but manual resume required.`,
        );
      }
    } catch (err) {
