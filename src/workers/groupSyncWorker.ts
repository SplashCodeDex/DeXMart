...
export function initializeGroupSyncWorker() {
  ...
  jobQueueService.process("group-sync", async (job: Job<GroupSyncJobData>) => {
    ...
    try {
      logger.info(
        `[GroupSyncWorker] Processing sync for ${groupJids?.length || 0} groups | Channel: ${channelId}`,
      );
      ...
      if (groupJids && groupJids.length > MAX_JIDS_PER_JOB) {
        logger.info(
          `[GroupSyncWorker] Large sync detected (${groupJids.length} groups). Splitting into sub-jobs of ${MAX_JIDS_PER_JOB}.`,
        );
        for (let i = 0; i < groupJids.length; i += MAX_JIDS_PER_JOB) {
          const chunk = groupJids.slice(i, i + MAX_JIDS_PER_JOB);
          await jobQueueService.addJob("group-sync", "sync-groups", {
            tenantId,
            channelId,
            groupJids: chunk,
            options,
      ...
      if (!result.success) {
        logger.warn(
          `[GroupSyncWorker] Skipping job: Channel ${channelId} not found or error: ${result.error?.message || "Unknown error"}`,
        );
        return;
      }
      ...
      try {
        socket =
          getWhatsAppRuntime().channel.whatsapp.getActiveWebListener() as unknown as ActiveChannel;
      } catch {
        // Runtime not yet initialised (no channel started)
      }
      if (!socket) {
        logger.warn(
          `[GroupSyncWorker] Skipping job: No active WhatsApp socket for channel ${channelId}`,
        );
        return;
      }
      ...
      logger.info(
        `[GroupSyncWorker] Successfully processed ${groupJids?.length || 0} groups for ${channelId}`,
      );
