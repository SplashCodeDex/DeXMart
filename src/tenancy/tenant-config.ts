...
export class TenantConfigService {
   ...
   */
  async resolveAgentChannelConfig(
    tenantId: string,
    channelId: string,
    agentId?: string,
  ): Promise<Result<ChannelConfig>> {
    try {
      const cacheKey = agentId
        ? `resolved:${tenantId}:${agentId}:${channelId}`
        : `resolved:${tenantId}:${channelId}`;
      ...
      const err = error instanceof Error ? error : new Error(String(error));
      logger.error(
        `[TenantConfigService] Failed to resolve Agent/Channel config for ${channelId}:`,
        err,
      );
      return { success: false, error: err };
    }
