...
describe("assertCan — hard gate failures (HTTP 402)", () => {
  it("throws 402 when Free user is at maxChannels limit", () => {
    ...
    expect(() => assertCan(guard.canStartChannel(), "channel", ctx)).toThrow(
      expect.objectContaining({ statusCode: 402, code: "PLAN_LIMIT_CHANNEL" }),
    );
  });
  ...
  it("throws 402 when Free user is at maxAgents limit", () => {
    ...
    expect(() => assertCan(guard.canCreateAgent(), "agent", ctx)).toThrow(
      expect.objectContaining({ statusCode: 402, code: "PLAN_LIMIT_AGENT" }),
    );
  });
  ...
  it("throws 402 when Free user exhausts monthly message quota", () => {
    ...
    expect(() => assertCan(guard.canSendMessage(), "message", ctx)).toThrow(
      expect.objectContaining({ statusCode: 402, code: "PLAN_LIMIT_MESSAGE" }),
    );
  });
  ...
  it("throws 402 when Free user tries to use a gated feature (campaigns)", () => {
    ...
    expect(() => assertCan(guard.hasFeature("campaigns"), "feature", ctx)).toThrow(
      expect.objectContaining({ statusCode: 402, code: "PLAN_LIMIT_FEATURE" }),
    );
  });
