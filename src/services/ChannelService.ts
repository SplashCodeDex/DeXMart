...
export class ChannelService {
  ...
  private getSharedNativeManager(): ChannelManager {
    ...
    const { channelLogs, channelRuntimeEnvs } = this.buildManagerOptions();
    this.sharedNativeManager = createChannelManager({
      loadConfig,
      channelLogs,
      channelRuntimeEnvs,
    });
    ...
    try {
      ...
      if (patch.updatedAt) {
        ...
        if (
          ...
          (current.data.updatedAt as any).toMillis() !== (patch.updatedAt as any).toMillis()
        ) {
          logger.warn(
            `Optimistic lock failure for channel ${channelId}: document was modified by another process.`,
          );
          return {
            success: false,
            error: new Error("DOCUMENT_STALE: Channel state was modified by another request."),
          };
        }
      }
  ...
  async moveChannel(
    ...
    try {
      ...
      await firebaseService.setDoc(
        this.getPath(targetAgentId),
        channelId,
        updatedChannel as any,
        tenantId,
      );
   ...
   */
  async findChannelByIdGlobally(
    channelId: string,
  ): Promise<Result<{ channel: Channel; tenantId: string; agentId: string }>> {
    try {
  ...
  async startChannel(
    ...
    if (existingStart) {
      logger.info(
        `[ChannelService] Channel ${channelId} is already starting, awaiting existing promise`,
      );
  ...
  private async _doStartChannel(
    ...
    try {
      ...
      if (isRunning) {
        ...
        logger.info(
          `[ChannelService] Force-restarting channel ${channelId} — stopping native channel`,
        );
  ...
  async stopChannel(
    ...
    try {
      ...
      if (!channelResult.success) {
        return {
          success: false,
          error: new Error("UNAUTHORIZED: Tenant does not own this channel."),
        };
      }
      ...
      this.updateStatus(tenantId, channelId, "disconnected", agentId).catch((err) =>
        logger.error(
          `[ChannelService] Failed to persist disconnected status for ${channelId}:`,
          err,
        ),
      );
  ...
  async deleteChannel(
    ...
    try {
      ...
      if (options.archive) {
        ...
        logger.info(
          `Channel deleted: ${channelId} from tenant ${tenantId} under Agent ${agentId} (with auth cleanup)`,
        );
      }
      ...
      for (const tenant of tenants) {
        ...
        for (const channel of channelsResult.data) {
          ...
          if (autoRestartStatuses.includes(channel.status)) {
            logger.info(
              `[ChannelService] Auto-starting channel ${channel.id} (${channel.name}) for tenant ${tenant.id}`,
            );

            // Non-blocking start
            this.startChannel(tenant.id, channel.id, channel.assignedAgentId || undefined).catch(
              (err) => {
   ...
   */
  async incrementChannelStat(
    tenantId: string,
    channelId: string,
    field: "messagesSent" | "messagesReceived" | "errorsCount",
    agentId: string = "system_default",
  ): Promise<void> {
    try {
      ...
      await firebaseService.setDoc(this.getPath(agentId), channelId, updateData, tenantId, true);
    } catch (err) {
      logger.error(
        `[ChannelService] Failed to increment stat ${field} for channel ${channelId}:`,
        err,
      );
    }
  }
  ...
  public async requestPairingCode(
    ...
    try {
      ...
      return {
        success: false,
        error: new Error("Pairing code not supported via native runtime yet"),
      };
    } catch (error: any) {
      return { success: false, error };
    }
