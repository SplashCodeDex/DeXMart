...
export class BackupService {
  ...
  async runBackup(
    tenantId: string,
    type: "full" | "database" | "media" = "database",
    ...
    try {
      ...
      if (!["pro", "enterprise"].includes(tenantData.plan || "starter")) {
        return failure(
          AppError.forbidden("Google Drive backups are restricted to Pro and Enterprise plans."),
        );
      }
      ...
      if (!tenantData.googleRefreshToken) {
        return failure(
          AppError.badRequest(
            "Google Drive integration is not connected. Please authorize in settings.",
          ),
        );
      }
      ...
      return failure(
        error instanceof AppError ? error : AppError.internal(metadata.error as string),
      );
    }
  }
