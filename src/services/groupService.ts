...
export class GroupService {
  ...
  createFunctions(
    ...
    return {
      ...
      approvePendingMembers: async (jids) =>
        this.handlePendingMembers(channel, groupJid, jids, "approve"),
      rejectPendingMembers: async (jids) =>
        this.handlePendingMembers(channel, groupJid, jids, "reject"),
  ...
  async syncDelta(
    ...
    try {
      ...
      logger.info(
        `[GroupService] Starting Delta-Sync for ${groupJids?.length || "all"} groups on ${channel.channelId}`,
      );

      const jidsToSync =
        groupJids && groupJids.length > 0
          ? groupJids
      ...
      for (let offset = 0; offset < jidsToSync.length; offset += CHUNK_SIZE) {
        ...
        for (const jid of chunk) {
          ...
          if (!options.force && existing) {
            const lastSynced =
              existing.syncedAt instanceof Timestamp
                ? existing.syncedAt.toDate()
                : existing.syncedAt
                  ? new Date(existing.syncedAt as any)
                  : new Date(0);
            const now = new Date();
            ...
            if (diffMinutes < 240) {
              logger.debug(
                `[GroupService] Skipping sync for ${jid} - last sync was ${Math.round(diffMinutes)}m ago`,
              );
              continue;
            }
          }
      ...
      logger.info(
        `[GroupService] Delta-Sync complete: ${jidsToSync.length} groups processed for ${channel.channelId}`,
      );
    } catch (error) {
