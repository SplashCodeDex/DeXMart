...
export class EmbeddingService {
  ...
  private constructor(keyManager: ApiKeyManager, initialKey?: string) {
    ...
    this.genAI = this.currentKey
      ? new GoogleGenerativeAI(this.currentKey)
    ...
    if (stats.totalKeys > 0) {
      logger.info(
        `EmbeddingService initialized with ${stats.totalKeys} API keys (${stats.healthyKeys} healthy)`,
      );
    } else {
  ...
  async generateEmbedding(text: string): Promise<Result<number[]>> {
    ...
    try {
      const data = await this.keyManager.execute(
        async (key) => {
      ...
      logger.error("[EmbeddingService] All keys exhausted or fatal error", {
        error: error.message,
      });
