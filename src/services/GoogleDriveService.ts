...
export class GoogleDriveService {
  ...
  private getOAuth2Client() {
    ...
    const redirectUri =
      ...
      `${config.get("NEXT_PUBLIC_APP_URL") || "http://localhost:3000"}/api/integrations/google/callback`;
      ...
      if (!tokens.refresh_token) {
        ...
        return {
          ...
          error: AppError.badRequest(
            "No refresh token returned. User may need to revoke access and try again.",
          ),
        };
      }
  ...
  public async uploadFile(
    ...
    try {
      ...
      return {
        success: false,
        error: AppError.internal("Failed to upload backup to Google Drive"),
      };
    }
  }
}
