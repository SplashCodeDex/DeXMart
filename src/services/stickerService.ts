...
export class StickerService {
  ...
  async emojiMix(
    emoji1: string,
    emoji2: string,
  ): Promise<Result<{ stickers: MixedSticker[]; message: string }>> {
    try {
      ...
      if (!response.data.results || response.data.results.length === 0) {
        return {
          success: false,
          error: new Error(`Mix Emoji ${emoji1}+${emoji2} Tidak Ditemukan!`),
        };
      }

      const stickers: MixedSticker[] = response.data.results
        .slice(0, 3)
        .map((r: any, i: number) => ({
          url: r.url,
  ...
  async createSticker(
    mediaBuffer: Buffer,
    options: { packname?: string; author?: string; categories?: string[] } = {},
