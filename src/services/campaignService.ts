...
export class CampaignService {
  ...
  async createCampaign(
    tenantId: string,
    data: Omit<Campaign, "id" | "tenantId" | "createdAt" | "updatedAt" | "stats" | "status">,
  ...
  async getCampaigns(tenantId: string): Promise<Result<Campaign[]>> {
    try {
      const campaigns = await firebaseService.getCollection<"users/{userId}/campaigns">(
        "campaigns",
        tenantId,
      );
  ...
  async startCampaign(tenantId: string, campaignId: string): Promise<Result<void>> {
    try {
      const campaign = await firebaseService.getDoc<"users/{userId}/campaigns">(
        "campaigns",
        campaignId,
        tenantId,
      );
  ...
  async resumeCampaign(tenantId: string, campaignId: string): Promise<Result<void>> {
    try {
      ...
      if (campaign.status !== "paused") {
        return {
          success: false,
          error: new Error(`Campaign is not paused (current status: ${campaign.status})`),
        };
      }
  ...
  async duplicateCampaign(tenantId: string, campaignId: string): Promise<Result<Campaign>> {
    try {
      ...
      const newCampaignData: Omit<
        Campaign,
        "id" | "tenantId" | "createdAt" | "updatedAt" | "stats" | "status"
      > = {
        ...data,
