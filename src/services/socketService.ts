...
export class SocketService {
  ...
  public initialize(server: HttpServer): void {
    ...
    this.io.use((socket: Socket, next) => {
      try {
        ...
        logger.security("Socket Auth: Connection failed", null, {
  ...
  public emitActivity(
    ...
    return this.emitToTenant(tenantId, "activity_event", {
      channelId,
      platform,
      type,
      message,
   ...
   */
  public emitBillingWarning(userId: string, capability: string, usedPercent: number): Result<void> {
    ...
    return this.emitToTenant(userId, "billing_warning", {
      ...
      message:
        messages[capability] ??
        `You're approaching a plan limit (${Math.round(usedPercent)}% used).`,
    });
  }
}
