...
export default class CommandSuggestionsService {
  ...
  async suggestCommands(input: string, recentCommands: any[] = [], options: any = {}) {
    ...
    const keywordMap = [
      ...
      {
        keywords: ["youtube", "yt", "audio", "mp3", "song", "music"],
        command: "youtubeaudio",
        category: "downloader",
        description: "Download YouTube audio as MP3",
      },
      {
        keywords: ["download", "link", "save"],
        command: "play",
        category: "downloader",
        description: "Find or download media from a link",
      },
      {
        keywords: ["image", "generate", "ai", "art", "picture"],
        command: "text2image",
        category: "ai-image",
        description: "Generate an image from text",
      },
      {
        keywords: ["translate", "language", "terjemah"],
        command: "translate",
        category: "tool",
        description: "Translate text",
      },
      {
        keywords: ["sticker", "stiker"],
        command: "sticker",
        category: "sticker",
        description: "Create a sticker from media",
      },
      {
        keywords: ["ocr", "text", "image"],
        command: "ocr",
        category: "tool",
    ...
    for (const c of candidates.values()) {
      ...
      const keywordBonus = keywordMap.find(
        (k) => k.command === c.command && k.keywords.some((kw) => text.includes(kw)),
      )
        ? 2
        : 0;
