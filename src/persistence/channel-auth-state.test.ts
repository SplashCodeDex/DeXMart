...
// ── Helpers ───────────────────────────────────────────────────────────────────
function makeMemStore(initial: Record<string, unknown> = {}): import('./channel-auth-state.js').AuthKeyValueStore {
...
describe('useChannelAuthState', () => {
  ...
  it('loads existing creds from store', async () => {
    const store = makeMemStore({ creds: { me: { id: 'user@s.whatsapp.net' }, signedIdentityKey: {} } });
    const { state } = await useChannelAuthState(store);
