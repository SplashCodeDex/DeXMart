  ...
  async handleCommonMessage(
    ...
    try {
      ...
      if (activeAgent && activeAgent.id !== "system_default" && isAiEnabled) {
        logger.info(
          `[Ingress] Routing ${message.platform} message to OpenClaw agent: ${activeAgent.name}`,
        );
        ...
        const rawConfig = ConfigModelGateSchema.parse(
          await loadConfigForUser(tenantId, null, db as unknown as UserConfigFirestore, loadConfig),
        ...
        try {
          const resolver = new UserContextResolverImpl(
          ...
          logger.warn(
            `[Ingress] Context resolution failed for tenant ${tenantId}, using full config fallback.`,
            err,
          );
        ...
        const baseMemoryResult = await getMemorySearchManager({
          cfg: userConfig as any,
