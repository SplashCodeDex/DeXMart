...
export class WebhookController {
  ...
  static async listWebhooks(req: Request, res: Response) {
    try {
      ...
      if (result.success) {
        ...
        res
          .status(500)
          .json({ success: false, error: result.error?.message || "Failed to fetch webhooks" });
      }
    } catch (error: any) {
  ...
  static async createWebhook(req: Request, res: Response) {
    try {
      const tenantId = req.user?.tenantId;
      const actor = Array.isArray(req.user?.userId)
        ? req.user.userId[0]
      ...
      if (result.success) {
        ...
        res
          .status(400)
          .json({ success: false, error: result.error?.message || "Failed to create webhook" });
      }
    } catch (error: any) {
  ...
  static async deleteWebhook(req: Request, res: Response) {
    try {
      ...
      if (result.success) {
        ...
        res
          .status(500)
          .json({ success: false, error: result.error?.message || "Failed to delete webhook" });
      }
    } catch (error: any) {
