...
describe("Multi-tenant Hierarchy Routes", () => {
  ...
  it("GET /agents/:agentId/channels should list channels for an agent", async () => {
    const mockChannels = [
      { id: "chan-1", name: "WhatsApp", type: "whatsapp", status: "connected" },
    ];
    (channelService.getChannelsForAgent as any).mockResolvedValue({
      success: true,
      data: mockChannels,
    });
  ...
  it("GET /channels/all should list all channels for a tenant", async () => {
    ...
    (channelService.getAllChannelsAcrossAgents as any).mockResolvedValue({
      success: true,
      data: mockChannels,
    });
