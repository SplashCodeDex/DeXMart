 ...
 */
router.post(
  ["/agents/:agentId/channels", "/channels", "/bots"],
  async (req: Request, res: Response) => {
    try {
      ...
      if (result.success) {
        ...
        channelService
          .startChannel(tenantId, result.data.id, agentId, true)
...
router.get("/agents/:agentId/channels", async (req: Request, res: Response) => {
  try {
    ...
    if (result.success) {
      ...
      logger.error(
        `getChannelsForAgent failed for tenant ${tenantId}, agent ${agentId}:`,
        result.error,
      );
      res.status(500).json({ success: false, error: result.error?.message });
    }
 ...
 */
router.get(
  ["/agents/:agentId/channels/:id", "/channels/:id"],
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.user?.tenantId as string;
      const agentId = (req.params.agentId || "system_default") as string;
      ...
      if (result.success) {
        ...
        res
          .status(404)
          .json({ success: false, error: result.error?.message || "Channel not found" });
      }
    } catch (error: any) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (result.success) {
        res.json({
          success: true,
          data: { message: `Channel ${archive ? "archived" : "deleted"} successfully` },
        });
      } else {
 ...
 */
router.post(
  ["/agents/:agentId/channels/:id/connect", "/channels/:id/connect"],
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.user?.tenantId as string;
      const agentId = (req.params.agentId || "system_default") as string;
 ...
 */
router.post(
  ["/agents/:agentId/channels/:id/disconnect", "/channels/:id/disconnect"],
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.user?.tenantId as string;
      const agentId = (req.params.agentId || "system_default") as string;
      ...
      if (result.success) {
        ...
        res
          .status(400)
          .json({ success: false, error: result.error?.message || "Disconnect failed" });
      }
    } catch (error: any) {
 ...
 */
router.get(
  ["/agents/:agentId/channels/:id/qr", "/bots/:botId/qr", "/channels/:id/qr"],
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.user?.tenantId as string;
      ...
      if (qrCode) {
        ...
        res.json({
          success: true,
          data: { qrCode: null, message: "QR code not yet available, please wait..." },
        });
      }
    } catch (error: any) {
 ...
 */
router.post(
  ["/agents/:agentId/channels/:id/pairing-code", "/bots/:botId/pairing-code"],
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.user?.tenantId as string;
      const channelId = (req.params.id || req.params.botId) as string;
      ...
      const result = await channelService.requestPairingCode(
        tenantId,
        channelId,
        phoneNumber,
        agentId,
      ...
      if (result.success) {
        ...
        res
          .status(400)
          .json({ success: false, error: result.error?.message || "Failed to get pairing code" });
      }
    } catch (error: any) {
...
router.post("/agents/:agentId/channels/:id/move", async (req: Request, res: Response) => {
  try {
    ...
    if (!tenantId)
      return res.status(401).json({ success: false, error: "Authentication required" });
    if (!targetAgentId)
      return res.status(400).json({ success: false, error: "Target Agent ID is required" });

    const result = await channelService.moveChannel(
      tenantId,
      channelId as string,
 ...
 */
router.get(
  ["/agents/:agentId/channels/:id/status", "/bots/:botId/status"],
  async (req: Request, res: Response) => {
    try {
      const tenantId = req.user?.tenantId as string;
      const channelId = (req.params.id || req.params.botId) as string;
