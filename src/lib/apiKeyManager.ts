...
import {
  ApiKeyManager as UniversalManager,
  ErrorClassification,
  LatencyStrategy,
} from "@splashcodex/api-key-manager";
import { z } from "zod";
...
export class ApiKeyManager {
  ...
  private constructor(apiKeys: string[]) {
    ...
    logger.info(
      `[ApiKeyManager] Initialized Universal Manager with ${apiKeys.length} keys (Semantic Cache Enabled)`,
    );
  }
  ...
  private wireEvents(): void {
    this.manager.on("keyDead", (key) =>
      logger.error(`[ApiKeyManager] Key PERMANENTLY DEAD: ...${key.slice(-4)}`),
    );
    this.manager.on("circuitOpen", (key) =>
      logger.warn(`[ApiKeyManager] Circuit OPEN (cooldown): ...${key.slice(-4)}`),
    );
    this.manager.on("keyRecovered", (key) =>
      logger.info(`[ApiKeyManager] Key RECOVERED: ...${key.slice(-4)}`),
    );
    this.manager.on("retry", (key, attempt, delay) =>
      logger.info(
        `[ApiKeyManager] Retrying with ...${key.slice(-4)} (Attempt ${attempt}, Delay ${delay}ms)`,
      ),
    );
    this.manager.on("fallback", (reason) =>
      logger.warn(`[ApiKeyManager] Triggering FALLBACK: ${reason}`),
    );
    this.manager.on("allKeysExhausted", () =>
      logger.error("[ApiKeyManager] ALL KEYS EXHAUSTED! No fallback available."),
    );
    this.manager.on("bulkheadRejected", () =>
      logger.warn("[ApiKeyManager] Bulkhead rejected request (concurrency limit reached)"),
    );
  }
  ...
  public static getInstance(): Result<ApiKeyManager> {
    ...
    if (!envValue) {
      logger.warn(
        "[ApiKeyManager] GOOGLE_GEMINI_API_KEY is not set. AI features will be disabled.",
      );
  ...
  public markFailedWithError(key: string, error: unknown): void {
    ...
    if (classification.markKeyFailed) {
      logger.warn(
        `[ApiKeyManager] Marking key failed: ${classification.type} (Retryable: ${classification.retryable})`,
      );
