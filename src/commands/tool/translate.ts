...
export default {
  ...
  code: async (ctx: MessageContext) => {
    ...
    try {
      ...
      const input =
        ctx.args.slice(ctx.args[0]?.length === 2 ? 1 : 0).join(" ") ||
        (ctx.quoted as any)?.content ||
        "";

      if (ctx.args[0]?.toLowerCase() === "list") {
        const langListUrl =
          "https://raw.githubusercontent.com/itsecurityco/to-google-translate/refs/heads/master/supported_languages.json";
        const response = await axios.get<Language[]>(langListUrl);
        const listText = response.data.map(
          (lang: Language) =>
