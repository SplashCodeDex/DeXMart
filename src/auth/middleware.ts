  ...
  try {
    ...
    try {
      ...
      if (isBlacklisted.success && isBlacklisted.data) {
        logger.security("Auth Middleware: Blacklisted token", null, {
        ...
        if (remaining < threshold) {
          ...
          if (ageSeconds > MIN_RENEWAL_AGE_SEC) {
            try {
              ...
              logger.warn(
                "Auth Middleware: Sliding window renewal failed, proceeding with current token",
                { error: renewalErr },
              );
            }
          }
    ...
    return res
      .status(500)
      .json({ success: false, error: "Internal server error in auth pipeline" });
  }
};
