  ...
  .object({
    ...
    plan: z.preprocess(
      (val, ctx) => {
        ...
        if (typeof val === "string" && ["starter", "pro", "enterprise"].includes(val.toLowerCase()))
        ...
        if (
          ...
          ["starter", "pro", "enterprise"].includes(legacyVal.toLowerCase())
        )
          return legacyVal.toLowerCase();
    ...
    status: z.preprocess(
      (val) => {
        if (typeof val === "string" && ["active", "suspended", "cancelled"].includes(val))
          return val;
...
export const ChannelSchema = z
  .object({
    ...
    type: z.preprocess(
      ...
      z
        .string()
        .refine((val) => !!getPlatformMetadata(val), { message: "Unsupported channel type" })
        .default("whatsapp"),
    ),
 ...
 */
export const CommandPermissionsSchema = z
  .object({
