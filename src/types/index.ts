...
export interface MessageContext {
  ...
  sendPresenceUpdate: (
    presence: "composing" | "recording" | "paused" | "unavailable",
    jid?: string,
  ) => Promise<void>;
  used: {
    command: string;
...
export interface GlobalContext {
  ...
  // DeXMart Fusion (Phase 4): unifiedAI replaced by OpenClaw's runEmbeddedPiAgent.
  // The GeminiAI class was deleted in Phase 1. IngressService now calls
  // runEmbeddedPiAgent() directly — no unifiedAI wrapper needed.
  // Field kept for backward compat with any code that checks context.unifiedAI.
  unifiedAI?: never;
