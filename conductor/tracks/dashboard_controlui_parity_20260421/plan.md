...
- [ ] Task 1.B.B.2: Implement `frontend/src/lib/gateway/gateway-rpc.ts` — `call<T>(method, params)` + `subscribe(event, handler)` + idempotency key support
- [ ] Task 1.B.C.1: Import Zod schemas directly from `src/gateway/server-methods/*` via workspace alias — NO duplicate schemas (PROJECT_RULES §0.1)
- [ ] Task 1.B.C.2: Create `frontend/src/lib/gateway/gateway-types.ts` — thin re-exports only, no redefinitions
- [ ] Task 1.B.D.1: Run tests green + `pnpm parity:check` green
- [ ] Task 1.B.D.2: Coverage ≥ 80% on `lib/gateway/*`
...
## Phase 2: Shared Primitives [checkpoint: pending]
...
- [ ] Task 2.1.A.1: Inventory every OKLCH token in `frontend/src/app/globals.css` — export to `docs/DASHBOARD_STYLE_GUIDE.md`
- [ ] Task 2.1.B.1: Write `docs/DASHBOARD_STYLE_GUIDE.md` — tokens, spacing scale, motion curves, glassmorphism recipe, Don't-Use list (emojis, custom hex)
- [ ] Task 2.1.B.2: Add `pnpm visual-diff` script — Playwright-visual or Chromatic-equivalent, stores baselines under `frontend/__visuals__/`
- [ ] Task 2.1.D.1: Run `pnpm visual-diff --update` to capture baselines for all 17 existing routes
...
- [ ] Task 3.1.A.1: Failing tests — ChatStore (Zustand) message/stream/abort state machine
- [ ] Task 3.1.A.2: Failing tests — `useChatSession` hook (send, stream append, abort, history load paginated)
- [ ] Task 3.1.B.1: `frontend/src/features/chat/` dir + `page.tsx` thin page + Sidebar nav entry
- [ ] Task 3.1.B.2: Implement `ChatStore`, `useChatSession`
