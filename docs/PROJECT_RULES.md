...
3. **Extensions are canonical** — `extensions/` contains the rich, battle-tested channel plugins (polls, reactions, media, pairing, directory). Channel features belong in `extensions/`, never duplicated in `src/services/channels/`.
4. **One channel engine** — OpenClaw's `createChannelManager()` + `PluginRegistry` is the **single** channel management system. `src/services/channels/whatsapp/WhatsappAdapter.ts`, DeXMart's `ChannelManager.ts`, and DeXMart's `registry.ts` are **deprecated dead-end code** pending removal.
5. **Frontend dominates** — The DeXMart Next.js dashboard is THE UI. ControlUI (OpenClaw's native web UI at `:18789`) is upstream heritage — **not user-facing**.
6. **Centralized elevation** — B2C tenancy, Stripe billing, Firebase auth, and Firestore persistence are injected into OpenClaw's **engine foundation** — not wrapped around it.
7. **One codebase** — Everything lives in `src/`. No `backend/src/` mirrors, no `openclaw/src/` references at runtime.
...
The following files exist in the codebase but are **deprecated** — they represent a parallel channel management system that violates Rule 0. They are pending removal in Phase 5:

- `src/services/channels/whatsapp/WhatsappAdapter.ts` — wraps OpenClaw's native extension instead of grounding B2C into the engine
